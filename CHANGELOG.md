...
* **Check FFXIV Playability (Tools menu)** - examines the file the way the game will play it: notes starting on the same tick on one performer (the game plays only one - previously you found these by exporting and hearing notes go missing in game), stacked duplicate notes, notes outside C3-C6, and track names matching no FFXIV instrument (in game, the track name is what selects the instrument). Held notes overlapped by later notes are deliberately not flagged - they are inaudible at game speed. Click an issue to select its notes in the editor and jump there, or select all offending notes at once and repair them with Delete Overlaps. The AI's `validate_ffxiv` runs the same engine and now reports every finding with its tick (it used to stop at the first overlap per track), tells apart stacked duplicates from chords, and quotes the legal instrument spellings when a track name is rejected.
