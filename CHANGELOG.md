...
### Summary
* **Check FFXIV Playability** - a workbench that reads the file the way the game plays it (chords, stacked duplicates, C3-C6 range, track names, voice peak, channel spread, empty tracks), selects the notes behind a finding, deletes exactly the marked ones in one undo step and hands the report to MidiPilot for a prioritised assessment (Phase 46).
* **Portable Mode and a home for every data file** - portable.txt or --portable keeps all settings in a config/ folder next to the exe; logs, soundfonts and custom prompts have platform-aware locations, which also unblocks the macOS build (Phase 45, issue #13).
* **MidiPilot answers editor questions from the manual** - two tools search the built-in manual and quote the page, and "Ask MidiPilot about the selection" in the note context menu sends the selected notes straight to the AI (Phase 44).
* **The AI arranges for FFXIV** - transpose_events, split_chords_to_tracks, copy_events_to_track, convert_tempo_preserve_duration and set_ffxiv_mode, an arrangement guide in the FFXIV context, and the two voice-load tools agree (Phase 46).
* **Prompt profiles can hide pitch bends from a single model** and the status bar shows an undo-memory readout; Auto-Fit Voice Load can select instead of remove (Phase 47).
* **GPU acceleration hardened** - closing during playback and mouse-wheel over the velocity lane no longer crash, the piano roll repaints reliably, view preferences survive exit, and the Performance page states which renderer is really active.
* **Bar numbering, tempo conversion and export range fixed** - the AI and the export dialog get the right bar count, the toolbar and status bar count beats by the meter, Convert Tempo refuses tempo-map modes for partial scopes, and deleting bars keeps the next downbeat (TEMPO-PARTIAL-REPLACE-001, EXPORT-MEASURE-001).
* **Settings that went to a dead scope now persist** - the MCP toolbar toggle honours port and token, the export dialog remembers its last settings, "MidiPilot Settings..." opens the MidiPilot page, and a model once flagged as tool-incapable is re-probed after seven days.

<details>
<summary>Full Changelog - FFXIV Playability, Portable Mode, Manual Bot</summary>
...
* **Check FFXIV Playability (Tools menu)** - a workbench that examines the file the way the game will play it and offers the repairs alongside. Checks (each individually selectable): **Chords** - several different pitches starting on the same tick on one performer, which the game rolls as a fast arpeggio (previously you found these by exporting and listening in game) - and **Stacked notes** - the same pitch twice on one tick, where the game plays it once and the duplicate is wasted. They are separate checks because they mean opposite things: a chord is often intentional, a stacked note almost always a defect, so the chords can be switched off to hunt duplicates. Further: notes outside C3-C6, track names matching no FFXIV instrument (in game, the track name is what selects the instrument), the raw voice peak against the 16-voice ceiling with notes/sec hotspots, channel spread (editor playback only) and empty instrument tracks. Held notes overlapped by later notes are deliberately not flagged - they are inaudible at game speed. Click a finding to select its notes and jump there; the buttons offer the matching repair (Delete colliding notes on the collisions, Channel Fixer, Auto-Fit) - selecting single findings, a Ctrl/Shift range or a whole group row such as "Stacked duplicates" scopes the delete to exactly those notes, from the button or the right-click menu - the window stays open and re-checks after every change, and "Analyze with MidiPilot" shows the AI's prioritized assessment right in the window. The AI's `validate_ffxiv` runs the same engine and now reports every finding with its tick, tells apart stacked duplicates from chords, and quotes the legal instrument spellings when a track name is rejected.
* **Portable Mode** - put a `portable.txt` next to `MidiEditorAI.exe` (or start with `--portable`) and ALL settings live in a `config/` folder beside the exe instead of the Windows registry; existing settings are copied over automatically on first portable start. A copy of the folder on a USB stick now carries everything: soundfonts, logs, prompts - and finally the configuration too. Without the marker nothing changes.
* **macOS build unblocked (issue #13)** - every data file (logs, soundfonts, custom prompts) now has a platform-aware home instead of being written next to the executable, which a signed macOS app bundle forbids. On Windows the paths are unchanged - everything stays next to the exe. The in-place auto-updater is Windows-only now; elsewhere the update check hands you to the releases page.
* **Ask MidiPilot about the editor itself - and get the manual's answer** - "How do I split drums?" used to get a guess; now the AI searches the built-in manual (two new tools, `search_help` and `get_help_section`), reads the matching section and answers from it, citing the page. The help database is generated from the manual pages themselves, so the answers cannot drift from the documentation, and everything works offline. MCP clients get the same two tools.
* **The AI now knows how to arrange for FFXIV, not just what is legal** - the FFXIV context gained an arrangement-craft section: guitar variant switches as a musical device (a lead switching Clean to Overdriven sounds like a real player - and how to place them), register choice when folding into range (distorted parts down an octave, not up two), deliberate density thinning as a tone decision, channel sharing, and the order of work (arrange, name tracks, channel fixer last, validate). External MCP clients get the same knowledge as a new `midi://ffxiv-guide` resource - previously they arranged without any of this guidance.
* **Three new AI arrangement tools: `transpose_events`, `split_chords_to_tracks`, `copy_events_to_track`** - the editor has had transpose, chord explode and note copying for years, but the AI tool surface had none of them: to move a part an octave the AI had to re-insert every note by hand. Now: transpose a track, range or the whole file by semitones (optionally folded octave-wise into the bard range C3-C6), split a chordal track voice-wise onto new tracks (voice 1 = highest - chords become monophonic performers), and copy notes to another track (double a melody, then transpose the copy). Each is one undo step.
* **The AI can manage FFXIV mode itself** - a new `set_ffxiv_mode` tool turns FFXIV Bard Performance mode on or off, `get_editor_state` reports the current state, and MCP clients now really receive the tool-list-changed notification when the mode flips (it was documented but never sent). Inside a MidiPilot agent run the switch takes effect from the very next step - the FFXIV tools and the FFXIV rules come into play (or leave) without starting a new request. Previously an external AI client could neither see that five more tools existed behind the FFXIV checkbox nor ask for them - you had to click the checkbox yourself.
* **New AI tool: `convert_tempo_preserve_duration`** - MidiPilot and MCP clients can now re-tempo material while keeping its real-time duration: event ticks are scaled and the tempo map updated so e.g. a 90 BPM vocal line drops into a 180 BPM project with the bars lining up again. Works on the whole file, chosen tracks or channels, or the current selection (partial scopes only re-tick their own events, so nothing outside the scope moves); the source tempo is auto-detected when not given. Always dry-run first with your confirmation, one undo step. This is the AI counterpart of the Convert Tempo dialog - same engine underneath.
* **Auto-Fit Voice Load can select instead of remove** - the tool's analysis is useful beyond deleting, so the dialog gained a **Select in editor** button: it closes and leaves the notes it found selected and unchanged, ready for any editor operation. On a dense tremolo run, set the skip slider to every 2nd note, press Select in editor and then Transpose Octave Up - and the machine-gun repeat becomes an alternating-octave arpeggio, in one undo step without clicking a single note by hand. The same works for moving the found notes to another track or changing their velocity.

* **Prompt Profiles: hide pitch-bend events from a single model** - a small model asked for a melody sometimes answers with one lonely pitch bend and no notes at all, over and over, however plainly you ask. The reason is that a pitch bend is the cheapest thing it can write: a note needs pitch, velocity, length and position, a bend needs almost nothing, so under pressure the model takes the shortest way out. The Prompt Profiles dialog now has a checkbox that takes the bend out of the tool list that model sees, so it has to write notes. It is set per profile, which means per model: tick it for the local model that does this and your other models are untouched. It applies to Agent mode, where the tool list exists, and never changes what external MCP clients receive.

* **Undo memory readout in the status bar** - a new status-bar field shows the active file's undo depth and a running estimate of how much memory the session's undo history has taken (e.g. "Undo 42 | ~118 MB"). Undo memory was previously invisible and can grow large in long sessions with big files - this makes it a number you can watch.

### Changed
* **The two voice-load tools finally agree** - `analyze_voice_load` reported the display model (release tails count as sounding, reading 24+ voices on a perfectly fine file) while `auto_fit_voice_load` judged raw concurrency, and an AI reading only the first would "fix" files that were never broken. The analysis now reports both numbers, says which is which, and bases its verdict on the raw truth the game enforces. Auto-Fit's AI tool also no longer thins 10% of every track by default - density thinning is now opt-in for the AI (the dialog keeps its default).

### Bug Fixes
* **GPU acceleration: two crashes** - with "Enable GPU acceleration for MIDI events" switched on, closing the window while a song was still playing crashed the app, and so did every mouse-wheel notch over the velocity/controller lane. Both are fixed. Because the crash on exit happened before the settings were written, it also lost the preferences of that whole session - so those now survive too.
* **GPU acceleration: three view preferences were lost on exit** - the grid division (Raster), View > Lock screen and the colouring choice (by channel / by track) were never written when the app was closed with GPU acceleration on, so they came back at their old values on every start. They now persist like every other view setting.
* **GPU acceleration: the piano roll could keep showing an old frame** - after pressing OK in Settings, after clicking a finding in the FFXIV playability window, after changing the grid division, and for a viewer following a live session, every other panel adopted the change while the piano roll stayed on the previous picture until the mouse moved over it. It now repaints with the change.
* **Settings: the GPU acceleration checkbox was lost when the dialog was closed with Esc** - ticking or unticking it (or pressing "Reset to Default") only took effect when the dialog was closed with the Close button; "Check for updates at startup" had the same gap. Both are now saved the moment they are changed. The Hardware Acceleration section also states which renderer is actually running - including that GPU acceleration stays inactive on a display with UI scaling and what to change for it - and "Enable software anti-aliasing" / "software smooth pixmap transforms" are no longer greyed out, because they shape the note grid in both renderers.
* **Status bar no longer shows the previous tab's numbers** - after switching tabs, the cursor/selection/chord fields kept the old document's values until the next edit. They now refresh with the switch.
...
* **Toolbar bar display showed the wrong time signature and beat** - in Bar mode the time display read "4/2" for a song in 4/4 and counted beats as half notes, so a 4/4 bar never got past beat 2. It now shows the real time signature and counts every beat in the bar. The status bar's "B:" field had the mirror-image problem - it divided every bar into four beats regardless of meter, so a 3/4 or 6/8 song showed beats that do not exist - and now follows the meter too.
...
* **Convert Tempo on one track retimed the whole file** - picking a partial scope (selected tracks, channels or events) together with "Replace with single fixed tempo at tick 0" or "Scale existing tempo map" rewrote the tempo map that the entire project plays through, so every note outside your scope moved too. The tempo map belongs to the whole file, so a partial scope now uses "Scale events only": the dialog selects it and greys the other two modes out with the reason, and the conversion engine refuses the combination whichever way it is asked - from the dialog or from the AI.
* **"MidiPilot Settings..." opened the wrong settings page** - the entry in the MidiPilot panel's gear menu (and the Open Settings button of the setup prompt) landed on the Midi I/O page, one click away from the settings it named. It now opens the dialog directly on the MidiPilot page.
* **The MCP toolbar toggle ignored the configured port and token** - the little MCP switch in the toolbar started the server on the default port with no authentication token, whatever you had entered under Settings, because it read from a settings scope nothing else writes to. It now starts exactly what the settings page configured. The same dead scope swallowed the export dialog's "remember last-used format, quality and folder" and the SoundFont helpers' mid-session save (your SoundFont choice could be lost if the app was closed by a crash) - all three now persist for real.
* **Deleting bars up to a meter change duplicated the time signature** - with the Measure tool, deleting bars 1-2 of a song whose meter changes at bar 3 left the old 4/4 AND the 3/4 both sitting at tick 0; the stale one won, the whole song was drawn and played with the wrong bar length, and both were saved. The moved meter now replaces the old one.
