...
* **Check FFXIV Playability (Tools menu)** - examines the file the way the game will play it: overlapping notes and stacked duplicates on a performer (the game silently drops these - previously you found them by exporting and hearing notes go missing in game), notes outside C3-C6, track-name/program mismatches that sound as the wrong instrument, and unknown instrument names. Click an issue to select its notes in the editor and jump there, or select all offending notes at once and repair them with Delete Overlaps. The AI's `validate_ffxiv` runs the same engine and now reports every finding with its tick (it used to stop at the first overlap per track), tells apart stacked duplicates from overlaps, flags program mismatches, and quotes the legal instrument spellings when a track name is rejected.
